...
export default async function recognitionWatcher(io) {
  try {
    ...
    changeStream.on("change", async (change) => {
      ...
      console.log("📄 Recognition:", recognition?.title, "Status:", recognition?.status);

      // ========== INSERT ==========
      if (change.operationType === "insert") {
        console.log("🆕 New recognition post:", recognition.title, "Status:", recognition.status);
      ...
      if (change.operationType === "update" || change.operationType === "replace") {
          ...
          switch(updatedFields.status) {
            ...
            case "archived":
              ...
              io.to(agentRoom).emit("recognitionArchived", {
                ...
                archivedAt: updatedFields.archivedAt || new Date().toISOString()
              });
            ...
            case "restored":
              console.log("♻️ Recognition restored from archive:", recognition.title);
        ...
        // ===== ENGAGEMENT UPDATES =====
        else if (updatedFields.likes || updatedFields.comments || updatedFields.views ||
                 updatedFields["engagement.likes"] || updatedFields["engagement.comments"]) {
          ...
          io.to(adminRoom).emit("adminEngagementUpdate", {
            recognitionId: recognition._id.toString(),
            title: recognition.title,
            totalLikes: recognition.engagement?.likes?.length || recognition.likes?.length || 0,
          ...
          if (recognition.status === "published") {
            io.to(agentRoom).emit("agentEngagementUpdate", {
              recognitionId: recognition._id.toString(),
              title: recognition.title,
              likes: recognition.engagement?.likes?.length || recognition.likes?.length || 0,
        ...
        // ===== CONTENT UPDATES =====
        else if (updatedFields.title || updatedFields.description || 
                 updatedFields.images || updatedFields.tags || 
                 updatedFields.recognitionType || updatedFields.employeeId ||
                 updatedFields.employeeName || updatedFields.department) {
    ...
    io.on("connection", async (socket) => {
      console.log("👤 User connected to recognition socket:", socket.id);
      ...
      socket.on("joinAgentRoom", () => {
        ...
        console.log(`👤 Agent joined agent room: ${socket.id}`);
      });
      ...
      socket.on("getAgentRecognitionData", async () => {
        ...
        try {
          const publishedRecognitions = await collection.find({ 
            status: "published" 
      ...
      socket.on("manualRecognitionPublished", (recognition) => {
        console.log("📢 Manual recognition publish from admin:", recognition._id, recognition.title);
      ...
      socket.on("manualRecognitionUpdated", (recognition) => {
        console.log("📝 Manual recognition update from admin:", recognition._id, recognition.title);
