  ...
  const fetchUserAchievements = () => {
    ...
    let achievements = allPosts.filter(post => {
      ...
      console.log(`Post ${post._id}:`, { 
        postEmployeeId, 
        currentUserId,
        match: postEmployeeId === currentUserId 
      });
      return postEmployeeId === currentUserId;
    });
    
    console.log('DEBUG - Found achievements:', achievements);
              ...
              <div className="mb-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-gray-900 text-xl">{userData.firstName} {userData.lastName}</h3>
                    <p className="text-gray-600">{userData.position || userData.role || 'Employee'}</p>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <div className="text-sm text-gray-500 mb-4">
                  ...
                  <p>Total posts in system: {allPosts?.length || 0}</p>
                </div>
...
const EmployeeBadges = ({ currentUserId, allPosts }) => {
  ...
  useEffect(() => {
    ...
    const fetchCurrentUser = () => {
      try {
        ...
        if (userData) {
          const user = JSON.parse(userData);
          console.log('User from localStorage (EmployeeBadges):', user);
          setCurrentUserData(user);
          
          // Calculate badge counts if we have posts
          if (allPosts && allPosts.length > 0) {
            console.log('All posts for badge calculation:', allPosts);
            
            const userRecognitions = allPosts.filter(post => {
              const postEmployeeId = post.employee?.employeeId;
              const userEmployeeId = user.employeeId;
              console.log(`Comparing - Post: ${postEmployeeId}, User: ${userEmployeeId}`);
              return postEmployeeId === userEmployeeId;
            });
            
            console.log('User recognitions found:', userRecognitions);
  ...
  console.log('EmployeeBadges render:', { 
    currentUserId, 
  ...
  return (
    <>
      <button
        onClick={() => {
          console.log('My Achievements button clicked!');
      ...
      >
        ...
        <div className="text-left">
          ...
          <div className="text-sm font-semibold text-gray-900">
            {badgeCounts.total > 0 ? `${badgeCounts.total} Awards` : 'View Awards'}
          </div>
        </div>
...
const PostDetailsModal = ({ post, isOpen, onClose}) => {
  ...
  const isCurrentUserRecipient = currentUserData?.employeeId === post.employee?.employeeId;
