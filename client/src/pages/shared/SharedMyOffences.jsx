      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-4">
            Offense Management
            {/* Offense Type Toggle */}
            <div className="bg-gray-200 rounded-full p-1 flex gap-1 shadow-inner flex-wrap">
              {[
                { type: "COACHING", count: unreadMyCoaching },
                { type: "IR", count: unreadMyOffenses },
              ].map(({ type, count }) => (
                <button
                  key={type}
                  onClick={() => {
                  ...
                  className={`relative px-3 py-1 rounded-full text-xs sm:text-sm font-medium transition-all flex items-center ${
                    offenseType === type
                      ? type === "IR"
                ...
                >
                  ...
                  {count > 0 && (
                    <span
                      className={`absolute -top-1 -right-1 w-4 h-4 text-[10px] flex items-center justify-center rounded-full font-semibold
            ${
              type === "IR" ? "bg-red-700 text-white" : "bg-blue-700 text-white"
            }`}
                    >
                      {count}
                    </span>
