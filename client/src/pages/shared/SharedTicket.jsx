...
import TicketRejectModal from "../../components/tickets/TicketRejectModal";
...
const TicketsTable = () => {
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      const ticketsWithDetails = await Promise.all(
        documents.map(async (ticket) => {
          try {
            ...
            if (
              detailData?.document &&
              Array.isArray(detailData.document) &&
              detailData.document.length > 0
            ) {
            ...
            console.error(
              `Failed to fetch details for ticket ${ticket.ticketNo}:`,
              error
            );
            return ticket;
          }
      ...
      ticketsWithDetails.sort(
        (a, b) => Number(b.ticketNo) - Number(a.ticketNo)
      );
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      console.log("testing responsee!", res);
  ...
  const handleRejectResolution = async (feedback) => {
    if (!ticketDetails || !user.email) {
      toast.error("Missing ticket information");
      return;
    }
    ...
    try {
      ...
      const newStatus =
        rejectResponse.data?.status || rejectResponse.data?.data?.status;

      if (newStatus?.toLowerCase() === "in progress") {
        toast.success("🚨 Ticket has been re-opened for IT review.");
      } else {
        toast.success("❌ Ticket rejection submitted successfully.");
      }
  ...
  return (
    <div>
      ...
      <button
        ...
        className="group flex items-center px-6 py-2.5 rounded-xl border border-[#800000]/30 bg-white text-[#800000] font-bold text-sm transition-all hover:bg-[#800000] hover:text-white active:scale-95 shadow-sm hover:shadow-lg"
      >
