...
const TeamLeaderDashboard = () => {
  ...
  // Initialize current user (Team Leader)
  useEffect(() => {
    const initializeTeamLeaderUser = async () => {
      try {
        console.log("🔄 Initializing team leader user...");
        ...
        if (token) {
          try {
            ...
            if (response.data && response.data.role === "team_leader") {
              console.log("✅ Team Leader authenticated:", response.data.name);
              setCurrentUser({
        ...
        // Fallback: Get first team leader from database
        try {
          const response = await api.get("/users/team-leaders");
          
          if (response.data && response.data.length > 0) {
            const teamLeaderUser = response.data[0];
            console.log("✅ Found team leader in database:", teamLeaderUser.name);
            setCurrentUser({
              _id: teamLeaderUser._id,
              name: teamLeaderUser.name,
              employeeId: teamLeaderUser.employeeId,
              department: teamLeaderUser.department,
              role: teamLeaderUser.role
            });
          } else {
            console.log("⚠️ No team leader found in database");
            setCurrentUser({
              _id: "team_leader_not_found",
              name: "Team Leader",
              employeeId: "TL001",
          ...
          setCurrentUser({
            _id: "team_leader_database_error",
            name: "Team Leader",
            employeeId: "TL001",
            department: "Operations",
            role: "team_leader"
        ...
        console.error("❌ Error initializing team leader user:", error);
        setCurrentUser({
          _id: "team_leader_error",
          name: "Team Leader",
          employeeId: "TL001",
          department: "Operations",
          role: "team_leader"
    ...
    initializeTeamLeaderUser();
  }, []);

  // Announcement interactions hook
  const { 
    hasLiked, 
    trackView, 
    handleLike, 
  ...
  // Update time ago for announcements
  useEffect(() => {
  ...
  const handleTogglePin = async (announcement) => {
    try {
      ...
      // Database update for pin status
      try {
  ...
  const handleReadMore = async (announcement) => {
    if (currentUser?._id) {
      console.log(`👁️ Team Leader ${currentUser.name} viewing announcement ${announcement._id}`);
  ...
  // Socket implementation for announcements
  useEffect(() => {
    console.log("🔄 Setting up socket listeners for team leader...");

    if (!socket) {
      console.log("❌ Socket not available for team leader");
      fetchAnnouncements();
      return;
    }

    if (!socket.connected) {
      console.log("❌ Socket not connected for team leader, attempting connection...");
      socket.connect();
    }
    ...
    const handleInitialData = (socketAnnouncements) => {
      console.log("📥 Received initial team leader data via socket:", socketAnnouncements?.length);
      ...
      if (Array.isArray(socketAnnouncements) && socketAnnouncements.length > 0) {
        ...
        setIsLoadingAnnouncements(false);
        dataLoaded = true;
        console.log("✅ Socket data loaded successfully for team leader");
      }
    };

    const handleAgentUpdate = (updateData) => {
      console.log("📊 Real-time team leader update:", updateData);
    ...
    const handleAnnouncementCancelled = (data) => {
      ...
      console.log("🗑️ Removing cancelled announcement from team leader view:", announcementId);
    ...
    console.log("📤 Requesting initial team leader data via socket...");
    socket.emit("getAgentData");

    const fallbackTimeout = setTimeout(() => {
      if (!dataLoaded) {
        console.log("⏰ Socket timeout for team leader, falling back to API...");
        fetchAnnouncements();
      }
    }, 3000);

    return () => {
      console.log("🧹 Cleaning up socket listeners for team leader");
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <span className="text-gray-600 text-lg">Loading team leader data...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 bg-gray-50">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Real-time team monitoring and announcement tracking
          </p>
      ...
      {/* Charts Grid - All Disabled with "COMING SOON" */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Team Activity Monitor</h3>
            <span className="text-xs font-medium px-2 py-1 bg-gray-100 text-gray-600 rounded">COMING SOON</span>
          </div>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
            <div className="w-32 h-32 sm:w-40 sm:h-40 flex-shrink-0 relative">
        ...
        {/* Work Hours Analysis - COMING SOON */}
        <div className="lg:col-span-3 bg-white rounded-xl p-6 shadow border border-gray-200 mt-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Work Hours Analysis</h3>
            <span className="text-xs font-medium px-2 py-1 bg-gray-100 text-gray-600 rounded">COMING SOON</span>
          </div>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
            <div className="w-full h-64 flex items-center justify-center bg-gray-50 rounded-lg border border-gray-200">
              <div className="text-center">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-r from-orange-700 to-orange-500 rounded-full flex items-center justify-center">
                  <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <span className="text-sm font-semibold text-gray-600">WORK HOURS ANALYSIS COMING SOON</span>
                <p className="text-xs text-gray-500 mt-2">Detailed work hour tracking will be available soon</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 bg-white rounded-xl p-6 shadow-lg border border-gray-200 mt-6">
          <div className="mb-6">
