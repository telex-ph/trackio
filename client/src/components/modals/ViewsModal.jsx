...
const ViewsModal = ({ isOpen, onClose, views, announcementTitle }) => {
        ...
        <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          {views && views.length > 0 ? (
                      ...
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-800 text-sm truncate">
                            {view.userName || 'Loading...'}
                          </p>
                          <p className="text-xs text-gray-500 truncate">
                            Viewed on {view.viewedAt ? DateTime.fromISO(view.viewedAt).toLocaleString(DateTime.DATETIME_MED) : 'Unknown date'}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 text-green-600 ml-2">
                        <Eye className="w-3 h-3 sm:w-4 sm:h-4" />
                        <span className="text-xs font-medium hidden sm:inline">Viewed</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-gray-500">
              <Users className="w-12 h-12 sm:w-16 sm:h-16 text-gray-400 mb-3" />
              <h4 className="text-lg font-semibold text-gray-600 mb-2 text-center">
                No Views Yet
              </h4>
              <p className="text-gray-500 text-center text-sm">
                This announcement hasn't been viewed by any users yet.
              </p>
            </div>
          )}
        </div>
