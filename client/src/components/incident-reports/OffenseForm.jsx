  ...
  const fetchEmployees = async (query) => {
    ...
    try {
      ...
      if (error.response && error.response.status !== 401) {
        showNotification("Failed to fetch employee suggestions. Please try again.", "error");
      }
      setSuggestions([]);
      setShowSuggestions(false);
    } finally {
    ...
    <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-6 sm:p-8 border border-white/20">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 ${isEditMode ? "bg-red-100" : "bg-indigo-100"} rounded-lg`}
          >
            {isEditMode ? (
          ...
          <div className="relative">
            ...
            {showSuggestions && (
              <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-2xl shadow-lg mt-1 max-h-40 overflow-y-auto">
                ...
                { suggestions.length > 0 && suggestions.filter(isValidSuggestion).length !== suggestions.length && (
                    <li className="p-3 text-gray-400 text-xs italic">Some results might be hidden due to incomplete data.</li>
                )}
              </ul>
            )}
          </div>
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => handleInputChange("offenseCategory", e.target.value)}
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {formData.offenseCategory &&
                offenseTypesByCategory[formData.offenseCategory]?.map((type, index) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => handleInputChange("offenseLevel", e.target.value)}
              className="w-full p-3 sm:p-4 bg-gray-50/50 border-2 border-gray-100 rounded-2xl focus:border-red-500 focus:bg-white transition-all duration-300 text-gray-800 text-sm sm:text-base"
            >
